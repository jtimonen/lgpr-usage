...
for(idx in 1:n_prot){
  ...
  tryCatch({
    ...
    cat('done!, max(Rhat) = ')
    fit       <- results$fit
    rel       <- fit@covariate_relevances$average
    mrh       <- max(fit@Rhat)
    selected  <- fit@covariate_selection$selected
...
cnam <- c("idx", "name", "selected", names(rel), "max(Rhat)", "n", "N", "N_cases", "N_b05", "N_b02", "flag")
inds <- 1:n_prot
result_df <- data.frame(inds, pnames, SEL, REL, MRH, n_data, N_indiv, N_cases, N_b05, N_b02, FLAG)
colnames(result_df) <- cnam

# Save results
saveRDS(result_df, file = "results_heter.rds")
