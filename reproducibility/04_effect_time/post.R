...
loadREL <- function(parentDir){
  ...
  FN    <- dir(path = parentDir, pattern = "res_")
  print(FN)
  L     <- length(FN)
  REL   <- array(0, c(L,7,3))
  REAL  <- matrix(0, L, 6)
...
# Create an ROC plot
createPlot <- function(roc1, roc2, roc3){
  ...
  method <- rep(c(paste("no prior, auc = ",    AUC[1], sep=""),
                  paste("exponential, auc = ",  AUC[2], sep=""),
                  paste("normal, auc = ",  AUC[3], sep="")),
                each = length(FPR)/3)
...
# Plot ROC curves
pA <- createPlot(roc_A1, roc_A2, roc_A3)
...
pB        <- createPlot(roc_B1, roc_B2, roc_B3) + labs(y = " ")#+ ggtitle('Disease covariate selection')

# Final ggplot
plot <- ggarrange(pA, pB, labels = c("a", "b"), ncol = 2)
ggsave(plot, file = "roc_uncrt.eps", width = 6, height = 3)
ggsave(plot, file = "roc_uncrt.png", width = 6, height = 3)
saveRDS(list(roc_A1,roc_A2,roc_A3, roc_B1, roc_B2, roc_B3), file = 'results_et.rds')
