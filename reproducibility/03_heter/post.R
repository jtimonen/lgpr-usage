...
get_res <- function(parentDir, n_aff){
  ...
  for(idx in idata){
      j  <- j + 1
      fn <- paste(parentDir, "/res_naff_", n_aff, "_", idx, sep ="")
      load(fn)
      REL[j,,1] <- res$r1
      REL[j,,2] <- res$r2
...
ggsave(plot, file = "panels_heter.eps", width = 12.5, height = 5.25)
ggsave(plot, file = "panels_heter.png", width = 12.5, height = 5.25)
REL <- list(r0,r2,r4,r6,r8)
ROC <- list(R0,R2,R4,R6,R8)
ret <- list(rel = REL, roc = ROC)
saveRDS(ret, file = 'results_het.rds')
