...
# Set paths
parentDir <- "../.."
resDir    <- paste(parentDir, "/A_basic/res", sep="")
...
inds <- 1:n_prot
result_df <- data.frame(inds, pnames, SEL, REL, MRH, n_data, N_indiv, N_cases, FLAG)
colnames(result_df) <- cnam

# Save results
saveRDS(result_df, file = "results_basic.rds")
